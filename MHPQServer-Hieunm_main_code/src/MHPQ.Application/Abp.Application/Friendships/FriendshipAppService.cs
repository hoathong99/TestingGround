...
namespace MHPQ.Friendships
{
    [AbpAuthorize]
    public class FriendshipAppService : MHPQAppServiceBase, IFriendshipAppService
    {
        ...
        public async Task<FriendDto> CreateFriendshipRequest(CreateFriendshipRequestInput input)
        {
            ...
            var sourceFriendship = new Friendship(userIdentifier, probableFriend, friendTenancyName, probableFriendUser.FullName, probableFriendUser.ProfilePictureId, FriendshipState.Requesting);
            ...
            var targetFriendship = new Friendship(probableFriend, userIdentifier, userTenancyName, user.FullName, user.ProfilePictureId, FriendshipState.Requesting);
        ...
        public void AcceptFriendshipRequest(AcceptFriendshipRequestInput input)
        {
            ...
            var senderClients = _onlineClientManager.GetAllByUserId(friendIdentifier);
            if (senderClients.Any())
            {
                _chatCommunicator.SendUserStateChangeToClients(senderClients, userIdentifier, FriendshipState.Blocked);
            }
        }
