...
{
    public class FriendshipManager : MHPQDomainServiceBase, IFriendshipManager
    {
        ...
        public void AcceptFriendshipRequest(UserIdentifier userIdentifier, UserIdentifier probableFriend)
        {
            ...
            var probableFriendship = GetFriendshipOrNull(probableFriend, userIdentifier);
            if (probableFriendship == null)
            {
                throw new ApplicationException("Friendship does not exist between " + probableFriend + " and " + userIdentifier );
            }
            //friendship.FriendUserName = 
            probableFriendship.State = FriendshipState.Accepted;
            UpdateFriendship(friendship);
        }
    }
}