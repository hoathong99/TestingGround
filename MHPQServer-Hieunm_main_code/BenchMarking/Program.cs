...
namespace Metrics
{
    ...
    class Benchmark
    {
        ...
        private static ILogger logger = loggerFactory.CreateLogger<Benchmark>();
                    ...
                    if (!settedThreshold)
                    {
                        ...
                        if (o != null)
                        {
                            ...
                            if (ll.Count > 2)
                            {
                                ...
                                logger.LogInformation("Config threshold time: " + string.Join(" ", th));
                                logger.LogInformation("Config threshold time: " + string.Join(" ", ll.ToArray()));
                                thresholdTime = th;
                            }
                        }
        ...
        static void Main(string[] args)
        {
            ...
            ILogger logger = loggerFactory.CreateLogger<Benchmark>();
            var services = new ServiceCollection();
            // ConfigureServices(services);
            services.AddLogging(configure => configure.AddConsole())
            .AddTransient<Benchmark>();
            ServiceProvider serviceProvider = services.BuildServiceProvider();

            //  logger = serviceProvider.GetService<ILoggerFactory>().CreateLogger<Benchmark>();;
            logger.LogInformation("Okkkkkkkkkkkk");

            Console.WriteLine("Hello World!xx");
            Benchmark mb = new Benchmark();

            Console.WriteLine(" " + mb.headcycle);

            ArrayList l = new ArrayList();
            TestBenchmark test = new TestBenchmark();
            for (int i = 0; i < 2; i++)
            {
                Thread t = new Thread(test.testLog);
                l.Add(t);
            }

            foreach (Thread s in l)
            {
            ...
            Console.WriteLine(String.Format("{0:###,##0.000}", 122223333.345678));

            Benchmark.config("500,800,1000,10000,100000,1000000");

            Console.WriteLine(TimeUtils.GetMilliseconds());
            Console.WriteLine(TimeUtils.GetNanoseconds());
            ...
            al.set(10);
            Console.WriteLine(al.get());


            Console.WriteLine(String.Format("{0:10|}", 122223333.345678));
